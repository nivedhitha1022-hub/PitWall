...
GITHUB_URL = "https://raw.githubusercontent.com/YOUR_GITHUB_USERNAME/YOUR_REPO_NAME/main/data/PitWall_Analytics_Dataset.xlsx"
    ...
    subs["tenure_months"] = subs.apply(
        lambda r: round(((r["churn_date"] if pd.notna(r["churn_date"]) else today) - r["signup_date"]).days / 30.44, 1)
        if pd.notna(r["signup_date"]) else np.nan, axis=1)
    subs["lifetime_revenue_usd"] = (subs["tenure_months"] * subs["monthly_price_usd"]).round(2)
    subs["nps_category"] = pd.cut(subs["nps_score"], bins=[-1, 6, 8, 10], labels=["Detractor","Passive","Promoter"])
    subs["age_group"]    = pd.cut(subs["age"], bins=[17,24,34,44,54,80], labels=["18-24","25-34","35-44","45-54","55+"])
    subs["cohort_month"] = subs["signup_date"].dt.to_period("M").astype(str)
    subs["plan_order"]   = subs["plan"].map({"Pit Lane":1,"Podium":2,"Paddock Club":3})
    ...
    sess["engagement_tier"] = pd.cut(sess["engagement_score"], bins=[-1,39,69,100], labels=["Low","Medium","High"])
    ...
    mrr["churn_rate_pct"] = (mrr["churned_subscribers"] / (mrr["active_subscribers"] + mrr["churned_subscribers"]).replace(0, np.nan) * 100).round(2)
    mrr["arpu_usd"] = (mrr["mrr_usd"] / mrr["active_subscribers"].replace(0, np.nan)).round(2)
    ...
    rfm  = subs[["subscriber_id","lifetime_revenue_usd","churned","plan","region","nps_score","tenure_months","monthly_price_usd","churned_bool","nps_category","age_group","acquisition_channel"]].merge(last_sess, on="subscriber_id", how="left").merge(freq, on="subscriber_id", how="left")
    ...
    def segment(row):
        ...
        elif r >= 3 and s >= 6: return "Potential Loyalist"
        elif r >= 3:            return "Recent"
