...
from theme import (
    base_layout, F1_RED, F1_WHITE, F1_SILVER, F1_GREY, F1_DGREY,
    F1_GOLD, F1_LGREY, ACCENT_BLUE, ACCENT_GREEN, ACCENT_AMBER,
    PLAN_COLORS, REGION_COLORS, SEGMENT_COLORS,
    insight, section_label
)


def rgba_fill(color, alpha=0.15):
    """
    Convert rgb() to rgba() safely for plot fills.
    Avoids Plotly hex transparency errors.
    """
    if "rgb" in color:
...
def render(subs, sess, mrr, rfm):
    ...
    avg_tenure = active["tenure_months"].mean()
    ...
    with col3:
        ...
        ).reset_index().sort_values("subscribers")
    ...
    with col5:
        ...
        fig6 = go.Figure(go.Bar(
            ...
            orientation="h",
            marker=dict(color=seg_summary["segment"].map(SEGMENT_COLORS))
        ))
    ...
    with i1:
        st.markdown(insight(
            ...
            f"<b>${champions['monetary'].mean():,.0f}</b>."
