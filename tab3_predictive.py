...
def build_features(subs, sess):
    ...
    df["region_enc"] = le_reg.fit_transform(df["region"].fillna("Unknown"))
    df["channel_enc"] = le_ch.fit_transform(df["acquisition_channel"].fillna("Unknown"))
    ...
    y = df["churned_bool"].fillna(0).astype(int)

    return X, y, df, features, feat_labels
...
def render(subs, sess, mrr, rfm):
    ...
    st.markdown("Random Forest churn model, risk scoring, and predicted 6-month value at risk.")
    st.markdown("---")

    X, y, df, features, feat_labels = build_features(subs, sess)
    ...
    with col1:
        ...
        fig.add_trace(
            go.Scatter(
                ...
                name="Random",
                mode="lines",
                line=dict(color=F1_SILVER, width=1, dash="dash"),
            )
        )
    ...
    with col3:
        ...
        layout3 = base_layout("Confusion Matrix", height=300)
    ...
    st.markdown("---")
    st.markdown(section_label("6-MONTH CLV FORECAST"), unsafe_allow_html=True)
    ...
    with col6:
        clv_region = (
            ...
            .agg(
                ...
                total_clv=("predicted_clv_6m", "sum"),
        ...
        fig6 = go.Figure(
            go.Bar(
                x=clv_region["region"],
                y=clv_region["total_clv"],
                marker=dict(
                    color=clv_region["avg_clv"],
                    colorscale=[[0, "#1C1C1C"], [0.4, "#FFB703"], [1, "#06D6A0"]],
                    showscale=True,
                    colorbar=dict(title="Avg CLV"),
                ),
                text=[f"${v:,.0f}" for v in clv_region["total_clv"]],
                textposition="outside",
                textfont=dict(color=F1_WHITE, size=10),
                hovertemplate="<b>%{x}</b><br>Total CLV: $%{y:,.0f}<br>Avg CLV: %{marker.color:.0f}<extra></extra>",
            )
        )

        layout6 = base_layout("Total Predicted CLV by Region", height=340)
        layout6["yaxis"]["title"] = "Total Predicted CLV (USD)"
        fig6.update_layout(**layout6)
        st.plotly_chart(fig6, use_container_width=True)

    st.markdown("---")
    st.markdown(section_label("30-DAY AT-RISK WATCHLIST"), unsafe_allow_html=True)
    ...
    table = at_risk[display_cols].copy()
    table.columns = [
        "Subscriber",
        "Plan",
        "Region",
    ...
    top20_clv = at_risk.head(20)["predicted_clv_6m"].sum()

    st.markdown(
        insight(
            f"🚨 <b>{len(at_risk)} high-risk active subscribers</b> represent <b>${total_at_risk_clv:,.0f}</b> in predicted 6-month value at risk. The top 20 alone account for <b>${top20_clv:,.0f}</b>."
        ),
    ...
    with col7:
        ...
        layout7 = base_layout("Churn Probability by Plan", height=320)
    ...
    with col8:
        ...
        fig8 = go.Figure(
            go.Pie(
                ...
                hole=0.5,
                marker=dict(
                    colors=[colors_risk.get(l, F1_SILVER) for l in risk_counts.index],
